...
#### 3. Create Virtual Environment (The Easy Way)
...
4. Select **Venv**.
5. Select **Python 3.8.x** (The version you just installed).
6. **Check the list:** Ensure `requirements.txt` is selected (checked) in the dependency list.
7. Click **OK**.

    - *VS Code will now create the `.venv` folder and install all libraries automatically. Wait for the progress bar in the bottom right to finish.*
