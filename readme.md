...
```
    |
    ├── *azure_copilot_etl.py  # 🧠 The Main Engine: Connects to Azure, Simulates AI, and Red Teams.
    ├── *generate_demo_data.py  # 🎭 The Storyteller: Automates creation of "Phase 1 Lite" demo scenarios.
    ├── *config.yaml  # ⚙️ Mission Control: Configure simulation rates, Azure creds, and Red Team settings.
    ├── *mock_expansion_data.json  # 🎨 Style Guide: Example JSON used by the AI to learn your corporate "Voice".
    ├── *roadmap.md  # 🗺️ The Master Plan: Architecture diagrams and development checklist.
    ├── *requirements.txt  # 📦 Dependencies: Python libraries needed to run the tools.
    └── generated_chats/             # 📤 Output Zone: Where your raw JSON logs appear.
        ├── baseline_data.json       #    -> The "Control" (Clean Data)
        └── comparator_data.json     #    -> The "Variable" (Risk Data)
    └── phase_1_lite_demo/           # 🎁 The Gift Shop: Pre-packaged "Story Mode" datasets generated by the demo script.
```
...
#### 🔴 Phase 3: The "Real World" (Discover Business Insights)
